...
REM  Add-MpPreference, C:\Programs 쓰기, C:\venvs 생성에 관리자 필요
REM ══════════════════════════════════════════════════════════════════
net session >nul 2>&1
if %errorlevel% neq 0 (
    echo [INFO] 관리자 권한이 필요합니다. UAC 창을 확인해주세요...
    echo [INFO] 원본 사용자: %USERNAME%
...
REM (관리자 계정의 %USERPROFILE%과 구분하기 위해)
if not "%~1"=="" (
...
if exist "%PYTHON_EXE%" (
    ...
    echo [INFO] %PYTHON_DIR% 에서 Python 3.11을 찾을 수 없습니다.
    echo [INFO] 설치를 시작합니다...
    ...
    if not exist "%PYTHON_EXE%" (
        echo.
        echo [ERROR] 설치 완료 후 python.exe를 찾을 수 없습니다:
        echo         %PYTHON_EXE%
        echo.
        pause
        exit /b 1
...
REM /XO : 원본이 대상보다 오래된 파일 제외 (대상에 이미 있는 파일 보호)
REM /XN : 원본이 대상보다 새로운 파일 제외 (대상에 이미 있는 파일 보호)
REM /XO + /XN 조합 = 대상에 이미 존재하는 파일은 날짜 무관하게 모두 건너뜀
REM /NP : 진행률 퍼센트 표시 안 함
robocopy "%VENV_SOURCE%" "%VENV_DIR%" /E /XO /XN /NP

REM robocopy errorlevel: 0-7은 성공(비트플래그), 8 이상은 실제 오류
if %errorlevel% geq 8 (
    echo.
    echo [ERROR] 파일 복사 중 오류 발생 (errorlevel: %errorlevel%)
    echo.
    pause
    exit /b 1
)
...
if not exist "%LAUNCHER%" (
    ...
    echo           %LAUNCHER%
    echo           바로가기는 생성합니다. 나중에 런처 파일 경로를 확인하세요.
)
